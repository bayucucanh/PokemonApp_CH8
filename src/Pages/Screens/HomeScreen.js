...
const HomeScreen = ({navigation}) => {
  ...
  const renderItem = ({item}) => (
      <TouchableOpacity style={styles.pokemon} onPress={() => GoToDetail(item.url)}>
        <Image source={PokeBall} style={{width: 30, height: 30, marginHorizontal: 15}}/>
  ...
  return (
    <ImageBackground source={BackgroundCatch} style={styles.container}>
      <FlatList
        numColumns={2}
        columnWrapperStyle={{ flex: 1, justifyContent: "space-between" }}        showsVerticalScrollIndicator={false}
        keyExtractor={(item, index) => index.toString()}
        data={pokeData.results}
        renderItem={renderItem}
      />
      <View style={styles.buttons}>
        <TouchableOpacity onPress={() => previousPokemon()}  style={[styles.btnPagination, {backgroundColor:'#e16c2c'}]}>
          <Text style={styles.btnText}>Previous</Text>
        </TouchableOpacity>
        <Text style={{ fontSize: 15, marginLeft: 13, fontWeight: 'bold', marginTop: 10 }}>{halaman}</Text>
        <TouchableOpacity onPress={() => nextPokemon()} style={[styles.btnPagination, {backgroundColor:'#97cce2'}]}>
